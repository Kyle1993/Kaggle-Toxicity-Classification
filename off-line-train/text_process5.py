...
if __name__ == '__main__':
    ...
    # load by pickle
    with open(glove_embedding_pkl_path, 'rb') as f:
        glove_embedding = pickle.load(f)
    with open(crawl_embedding_pkl_path, 'rb') as f:
        crawl_embedding = pickle.load(f)

    # bert_vocab_oath = 'vocab.txt'
    # bert_vocab = load_vocab(bert_vocab_oath)

    print('Processing...')
    x_train = tp.process(train['comment_text'], set(glove_embedding.keys()) | set(crawl_embedding.keys()))
    x_test = tp.process(test['comment_text'],set(glove_embedding.keys()) | set(crawl_embedding.keys()))

    # x_train = tp.process(train['comment_text'], bert_vocab,lower=True)
    # x_test = tp.process(test['comment_text'],bert_vocab,lower=True)
    ...
    print('vocb_size:',len(tokenizer.word_index))
    ...
    print('unknown words: ', len(unknown_words_glove))

    with open('embedding.pkl', 'wb') as f:
        pickle.dump({'embedding': embedding_matrix, 'unknown_words': unknown_words}, f)

    print('All Time Used:',datetime.now()-start_time)

    vocab = tp.build_vocab(list(x_train)+list(x_test))

    print('Glove:')
    oov_glove = tp.check_coverage(vocab, set(glove_embedding.keys()))
    print('Crawl')
    oov_crawl = tp.check_coverage(vocab, set(crawl_embedding.keys()))
    print('After merge:')
    oov_whole = tp.check_coverage(vocab, set(glove_embedding.keys()) | set(crawl_embedding.keys()))

    # oov_whole = tp.check_coverage(vocab, bert_vocab)
